...
namespace NovaArena.Core
{
    ...
    public sealed class InputRouter : IInputRouter
    {
        ...
        public void Tick(float deltaTime)
        {
            ...
            var aimInput = _actions.Gameplay.Aim.ReadValue<Vector2>();

            if (Mouse.current != null && Mouse.current.delta.IsActuated())
            {
                aimInput *= _config.MouseSensitivity;
                if (_config.InvertMouseY)
                {
                    aimInput.y = -aimInput.y;
                }
            }

            _aim = Vector2.Lerp(_aim, aimInput, deltaTime * _config.AimSmoothing);
        }
